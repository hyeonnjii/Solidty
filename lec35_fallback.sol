...
// 이 예시는 contract you 가 contract bank에게 이더 보내는 예시
contract Bank{

    event JustFallbackWIthFunds(address _from,uint256 _value ,string message);
    //~0.6 
  function() external payable {  // fallback 함수로 이더를 받게 됨
...
contract You{
    ...
    function DepositWithSend(address payable _to) public payable{
         bool success = _to.send(msg.value); // send를 이용해 bank 로 이더를 보내
    ...
    function JustGiveMessage(address _to) public{
        // ~ 0.7
        (bool sent, ) = _to.call("HI"); // call 은 함수를 보내는 기능도 있음 > 하지만 bank 컨트랙트에는 hi 라는 함수가 없어서 fallback 에 걸릴 것
        require(sent,"Failed to send either");
